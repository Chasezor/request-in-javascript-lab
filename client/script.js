...
let button = document.querySelector('#say-hello-button')
...
button.addEventListener('mouseover', buttonStyle)
...
function buttonStyleOff(){
    console.log('over the button')
    button.style.backgroundColor = "#EFEFEF"
    button.style.color = "black"
}

button.addEventListener('mouseout', buttonStyleOff)
...
const ohMy = () => {
    // YOUR CODE HERE
    axios.get(`${baseURL}/animals`).then((res) =>{console.log(res.data)})
}
...
const queryFun = () => {
    ...
    axios.get(`http://localhost:3000/query-test/?myquery=num-of-people`)
    .then((response) => {
        let repeatText = document.getElementById('repeat-text');
        repeatText.style.display = 'block';
